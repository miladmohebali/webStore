...
import { logout } from "../../features/slices/authSlide";
  ...
  return (
    <>
      ...
      <div className="flex justify-around items-center">
        <div>
          <img className="w-full h-27" src={logo} alt="store" />
        </div>
        <div className="flex flex-row items-center">
          <button
            onClick={() => dispatch(logout())}
            className="font-inter text-base font-bold tracking-normal leading-none text-center mr-4"
          >
            خروج
          </button>
          <svg
          ...
          >
            ...
            <div>{open && <Cart open={open} handleOpen={setOpen}></Cart>}</div>
          </div>
          <div>
            {image && <Avatar src={image} size="sm" className="mr-2"></Avatar>}
            Hi {name}
          </div>
        </div>
      </div>
